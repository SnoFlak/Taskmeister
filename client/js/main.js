console.log('Obtaining information about the calendar from the rest server.');

let calendar = null;

fetch('http://localhost:3030/api/v1/events/get', {
  method: 'GET',
  headers: {
  ...
  .then((data) => {
    // The returned data is set to the calendar object.
    calendar = data;
    render();
  })
  .catch((error) => {
...
const eventModalBuilder = (date) => {
  ...
  eventModalBodyDescription.textContent =
    'Schedule an event to be viewed on the calendar.';
  ...
  eventModalBodyFormButtonCancel.classList.add(
